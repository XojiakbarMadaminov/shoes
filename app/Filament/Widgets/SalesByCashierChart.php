...
class SalesByCashierChart extends ChartWidget
{
    ...
    protected ?string $heading = 'Sotuvlar kassirlar bo‘yicha';
    ...
    protected function getData(): array
    {
        ...
        $totals = Sale::query()
            ...
            ->selectRaw('created_by, SUM(total_amount) as total_amount, COUNT(*) as sales_count')
            ->groupBy('created_by')
            ->get()
            ->keyBy('created_by');
        ...
        foreach ($totals->values() as $index => $row) {
            $cashierName = $userNames[$row->created_by] ?? 'Nomaʼlum';
            $labels[]    = sprintf(
                '%s — %s soʼm (%d ta)',
                $cashierName,
