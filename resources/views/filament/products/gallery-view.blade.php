    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-end">
            <label class="w-full">
                ...
                <input type="search" wire:model.live.debounce.400ms="search"
                       placeholder="Mahsulot nomi yoki bar kodi..."
                       class="w-full rounded-xl border border-gray-200 bg-white/80 px-4 py-2 text-sm shadow-sm outline-none transition focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
            </label>
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:gap-4 lg:grid-cols-5 xl:grid-cols-6">
            @forelse ($products as $product)
                <div
                    class="group relative flex flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-sm transition hover:-translate-y-1 hover:border-primary-500 hover:shadow-lg dark:border-gray-700 dark:bg-gray-900 md:p-4">
                    ...
                    <div
                        class="mb-3 h-36 w-full overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800 sm:h-40 md:h-48">
                        @if ($mediaCount > 1)
                            {{-- KARUSEL --}}
                            <div
                                x-data="{
                            ...
                            >
                                @foreach ($mediaItems as $index => $media)
                                    <img
                                        ...
                                        @click="
                                            window.zoomModal.urls = @js($imageUrls);
                                            window.zoomModal.i = {{ $index }};
                                            $dispatch('open-zoom');
                                        "
                                        x-cloak
                    ...
                    <a href="{{ route('filament.admin.resources.products.view', $product) }}"
                       class="flex flex-1 flex-col justify-between">
                        <div>
                            <div
                                class="text-sm font-semibold text-gray-900 group-hover:text-primary-600 dark:text-white dark:group-hover:text-primary-400 line-clamp-2 md:text-base">
                                {{ $product->name }}
                            </div>
                        </div>
                    </a>
            ...
            @empty
                <div
                    class="col-span-2 rounded-2xl border border-dashed border-gray-300 bg-white p-8 text-center text-sm text-gray-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 sm:col-span-3 lg:col-span-5 xl:col-span-6">
                    Mahsulot topilmadi.
                </div>
            @endforelse
        </div>
        ...
        <div class="relative w-full h-full flex items-center justify-center" @click.stop>
            <template x-if="urls.length > 0">
                <div class="relative">
                    ...
                    {{-- Counter --}}
                    <div
                        class="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/60 text-white px-3 py-1 rounded-full text-sm backdrop-blur-sm"
                        x-show="urls.length > 1" x-text="(i+1) + ' / ' + urls.length"></div>
                </div>
            </template>
